...
@section('page-subtitle', 'Lista de Usuarios')
	...
	<div class="col-sm-12">
		<div class="card">
			<div class="card-header border-0 pt-5">
				...
				<div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="Da clic para crear un nuevo Usuario">
    ...
    <div class="modal-dialog">
        <form class="modal-content" id="form" action="#">
            ...
            <div class="modal-body">
            	@method('POST')
            	<x-forms.input mb="mb-3" placeholder="Nombre" name="name" />
            	<x-forms.input mb="mb-3" placeholder="Email" type="email" name="email" />
            	<x-forms.input mb="mb-3" placeholder="Contraseña" type="password" name="password" />
            </div>
...
<script>
	...
	KTUtil.onDOMContentLoaded(function() {
		...
		openModal.addEventListener('click', (evt) => {
			theForm.action = '{{route("usuarios.store")}}';
			theForm.reset();
			theFormTitle.innerHTML = 'Nuevo Usuario';
		...
		TTValidate(theForm, {					
			...
			'password': {
				validators: {
					...
					stringLength: {max:32,message: 'El campo debe tener máximo 32 caracteres.'},
				}
			},
		}, null, () => {
			checker = false;
...
<x-datatable id="table" :route="route('usuarios.index')">
	[{data: 'id', name: 'id'},
	{data: 'name', name: 'name'},
	...
	<x-slot name="callback">
		...
		TTModalForm.init('{{route('usuarios.update', ['user' => 'content'])}}', modal, modalContainer, '.update', '{{route('usuarios.show', ['user' => 'content'])}}', 'Editar Usuario');
	</x-slot>
</x-datatable>
@endpush